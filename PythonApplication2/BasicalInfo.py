...
class userDetailCollection(wx.Frame):
    def __init__(self, parent, id):# this is the constructor function to build the register interface
        ...
        frame = wx.Frame.__init__(self, parent, id, title="Set basical detail", pos=(500, 250), size=(800,500))#initialize the main form
        panel = wx.Panel(self)
        ...
        self.bt_submit = wx.Button(panel, label='Submit', pos=(100, 220))
        self.bt_cancel = wx.Button(panel, label='Cancel', pos=(190, 220))
        self.bt_resit = wx.Button(panel, label='Resit', pos=(280, 220))
        self.bt_TandC = wx.Button(panel, label='Term and Conditions', pos=(100, 250))
        self.checkBox = wx.CheckBox(panel, label = "I agree all Term and Conditions", pos= (100, 280))

        self.bt_submit.Bind(wx.EVT_BUTTON, self.OnclickSubmit)
        self.bt_cancel.Bind(wx.EVT_BUTTON, self.OnclickCancel)
        self.bt_resit.Bind(wx.EVT_BUTTON, self.OnclickResit)
        self.bt_TandC.Bind(wx.EVT_BUTTON, self.OnclickTandC)
    ...
    def OnclickSubmit(self, event):
        ...
        if username == "" or password == "" or name=="" or gender=="" or age=="" :
            ...
            message = "You should agree Term and Conditions first"
        else: 
            try:
